(ns org.parkerici.enflame.library.save
  (:require [org.parkerici.enflame.library.item :as i]
...
(defn save [description]
  (let [item
        {::i/blockdef (bo/workspace-xml-string)
         ::i/date-created (now)
;         ::i/output-kind 
         ::i/query @(rf/subscribe [:text])
         ::i/description description
         ::i/image (workspace-image)
         }]
    (do-save item)
    ))
...
(rf/reg-event-db
 ...
 (fn [db [_ key]]
   (api/ajax-get "/api/library/get"
                 {:url-params {:key key}
                  :handler #(rf/dispatch [:load-item %1])})
   db))

(rf/reg-event-db
 :load-item
 (fn [db [_ item]]
   (blockly/restore-from-saved (::i/blockdef item))
   db))
